...
// 获取我的代码片段
// 对应网关: GET /api/v1/notes/me/pastes
export const listMySnippets = () => {
  return request.get<unknown, RawSnippet[]>('/api/v1/notes/me/pastes').then((list) => list.map(normalizeSnippet))
}

// 获取代码片段详情
// 对应网关: GET /api/v1/notes/pastes/:id
export const getSnippet = (id: string) => {
  return request.get<unknown, RawSnippet>(`/api/v1/notes/pastes/${id}`).then(normalizeSnippet)
}

// 创建代码片段
// 对应网关: POST /api/v1/notes/pastes
export const createSnippet = (data: SaveSnippetRequest) => {
  return request.post<unknown, RawSnippet>('/api/v1/notes/pastes', data).then(normalizeSnippet)
}

// 更新代码片段
// 对应网关: PUT /api/v1/notes/pastes/:id
export const updateSnippet = (id: string, data: SaveSnippetRequest) => {
  return request.put<unknown, RawSnippet>(`/api/v1/notes/pastes/${id}`, data).then(normalizeSnippet)
}
