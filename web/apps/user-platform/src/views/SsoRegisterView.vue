...
const ssoContext = computed(() => {
  ...
  const appCode = getSingleQueryValue(route.query.app_code).trim() || clientId || 'go-note'
  ...
  const hasAppContext = appCode.length > 0 && redirectUri.length > 0 && isSafeRedirectUri(redirectUri)
